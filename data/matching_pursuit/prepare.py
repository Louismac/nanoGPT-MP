...
train_data.tofile(os.path.join(os.path.dirname(__file__), 'train_x.bin'))
val_data.tofile(os.path.join(os.path.dirname(__file__), 'val_x.bin'))

train_data = np.vstack([sparse[:int(n*0.9)] for i in range(100)])
val_data = np.vstack([sparse[int(n*0.9):] for i in range(100)])
train_data = np.array(train_data, dtype=np.float32)
val_data = np.array(val_data, dtype=np.float32)
train_data.tofile(os.path.join(os.path.dirname(__file__), 'train_y.bin'))
val_data.tofile(os.path.join(os.path.dirname(__file__), 'val_y.bin'))
